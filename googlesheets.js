...
function displayProductsByType(productsByType, tagId) {
  ...
  productsByType.forEach((p) => {
    let buttonHTML = `<button id="button-add" class="bg-gradient-to-r from-green-500 to-blue-500 rounded-full text-gray-50 self-center py-2 px-4 flex flex-row hover:from-green-500 hover:to-green-500" onclick="add(${p.id}, ${p.price})>
